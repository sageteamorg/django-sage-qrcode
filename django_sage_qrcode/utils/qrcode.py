...
def add_text_to_image(image, text):
    ...
    draw.text(((image.width - text_width) // 2, image.height - text_height - 10), text, fill='black', font=font)
    return image
...
def add_frame_to_image(image, frame_type='simple'):
    ...
    if frame_type == 'simple':
        ...
        draw.rounded_rectangle([(0, 0), image.size], radius=20, outline="black", width=10)
    return image
